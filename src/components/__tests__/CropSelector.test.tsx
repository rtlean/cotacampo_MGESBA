...
describe('CropSelector Component', () => {
  it('deve renderizar as 4 culturas agrícolas atendidas com seus nomes e ícones', () => {
    render(<CropSelector selectedCrops={[]} onChange={() => {}} />);

    expect(screen.getByRole('checkbox', { name: /Café/i })).toBeInTheDocument();
  ...
  it('deve alternar a seleção via clique adicionando ou removendo culturas', () => {
    ...
    const { rerender } = render(
      <CropSelector selectedCrops={['cafe']} onChange={handleChange} />
    );

    // Clicar em Pimenta deve chamar onChange com ['cafe', 'pimenta']
    const pimentaBtn = screen.getByRole('checkbox', { name: /Pimenta-do-reino/i });
    fireEvent.click(pimentaBtn);
    expect(handleChange).toHaveBeenCalledWith(['cafe', 'pimenta']);

    // Clicar em Café (já selecionado) deve chamar onChange removendo-o
    const cafeBtn = screen.getByRole('checkbox', { name: /Café/i });
    fireEvent.click(cafeBtn);
    expect(handleChange).toHaveBeenCalledWith([]);

    // Rerender com nova lista
    rerender(<CropSelector selectedCrops={['cafe', 'pimenta']} onChange={handleChange} />);
